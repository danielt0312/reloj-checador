...
<div class="container">
	...
	<label class="page-container__title ">&nbsp;Administración: Menú</label>
	<br>
  <a href="<?=base_url()?>Administracion/Menu/Agregar" class="btn btn-azul" >/ Agregar Menú</a>
<hr>
...
<script type="text/javascript">
  $(document).ready(function() {
      ...
      var table = $('#example').DataTable(
        {
          ...
          'processing': true,
              ...
              'serverMethod': 'post',
              'ajax': {
                  'url':'<?=base_url()?>Administracion/Menu/Contenido',
                  'type': "POST",
                  'data':function(data) {
                    data.id = $('#datepicker1').val();
                    data.nombre = $('#datepicker2').val();
                    data.url = $('#datepicker3').val();
                    data.orden = $('#datepicker4').val();
                  },
              },
            'columns': [
                 { data: 'id' },
      ...
      $('#search').on( 'click change', bfunction (event) {
...
<div id="user-id" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <form class="validacionGeneral" action="<?=base_url('Administracion/Menu/Eliminar')?>" method="POST" onKeypress="if(event.keyCode == 13) event.returnValue = false;">
