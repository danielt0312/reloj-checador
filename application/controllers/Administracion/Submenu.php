...
class Submenu extends CI_Controller {
	function __construct() {        
        ...
        $this->load->model('Administracion/M_Submenu');
        $this->seguridad();      
    }
    public function index() {      
        ...
        $this->load->view('Administracion/TablaSubmenu');
    ...
    public function Contenido(){
         ...
         // Get data
         $data = $this->M_Submenu->getEmployees($postData);
    ...
    public function Agregar() {      
        ...
        $this->load->view('Plantilla/v_head');
        $this->load->view('Plantilla/v_nav',$data_nav);
        $this->load->view('Administracion/AgregarSubmenu');
        $this->load->view('Plantilla/v_footer');
    ...
    public function Editar($id) {      
        $data_nav['menu']           = $this->menu();
        $data_nav['submenu']        = $this->submenu();
        $data['consulta']= $this->Querys->consulta('*', 'rh_submenu','','id='.$id);   
        $this->load->view('Plantilla/v_head');
        $this->load->view('Plantilla/v_nav',$data_nav);
        $this->load->view('Administracion/EditarSubmenu',$data);
        $this->load->view('Plantilla/v_footer');

    }
    public function Eliminar() {      
        $id=$this->input->post('id');
        $this->M_querys->actualiza('rh_submenu', array('alta_baja' => 0,'fecha_modificacion' => $this->input->post('fecha_modificacion')),'id='.$id);
    ...
    public function Guardar($id=null){
        ...
        if ($id) {
            $insert_id = $this->M_querys->guardar('rh_submenu', $data, $id);
            echo $this->session->flashdata('mensaje');
        } 
        else {
            $insert_id = $this->M_querys->guardar('rh_submenu', $data);
            echo $this->session->flashdata('mensaje');
        }              
    }
    /**********************Permisos***************************/
