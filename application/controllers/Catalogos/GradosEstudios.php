...
class GradosEstudios extends CI_Controller {
	function __construct() {        
        parent::__construct();
        date_default_timezone_set('America/Mexico_City');
        $this->load->model('Catalogos/M_GradosEstudio');
        $this->seguridad();  
    }
    public function index() {      
        ...
        $this->load->view('Catalogos/GradosEstudios/Registros');
    ...
    public function Contenido(){
         ...
         // Get data
         $data = $this->M_GradosEstudio->getEmployees($postData);
    ...
    public function Agregar()
    {
        ...
        $this->load->view('Plantilla/v_head');
        $this->load->view('Plantilla/v_nav',$data_nav);
        $this->load->view('Catalogos/GradosEstudios/Agregar');
        $this->load->view('Plantilla/v_footer');
    }
    public function Editar($id)
    {
        $data['consulta']= $this->Querys->consulta('*', 'rh_cat_gradosestudio','','id='.$id);
        $data_nav['menu']           = $this->menu();
        $data_nav['submenu']        = $this->submenu();   
        $this->load->view('Plantilla/v_head');
        $this->load->view('Plantilla/v_nav',$data_nav);
        $this->load->view('Catalogos/GradosEstudios/Editar',$data);
        $this->load->view('Plantilla/v_footer');
    }
    public function Eliminar(){
        $id=$this->input->post('id');
        $this->M_querys->actualiza('rh_cat_gradosestudio', array('alta_baja' => 0,'fecha_modificacion' => $this->input->post('fecha_modificacion')),'id='.$id);        
        //$this->index();
    ...
    public function Guardar($id=null){
        ...
        if ($id) {
            $insert_id = $this->M_querys->guardar('rh_cat_gradosestudio', $data, $id);
            echo $this->session->flashdata('mensaje');
        } 
        else {
            $insert_id = $this->M_querys->guardar('rh_cat_gradosestudio', $data);
            echo $this->session->flashdata('mensaje');
        }              
    }
    /**********************Permisos***************************/
