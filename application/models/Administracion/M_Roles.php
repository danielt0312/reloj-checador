...
class M_Roles extends CI_Model {
  ...
  function getEmployees($postData=null){
    ...
    if ($searchById !='' && $searchByName !='') {
     ...
     $records = $this->db->get('rh_roles')->result();
     ...
     foreach($records as $record ){
        ...
        $data[] = array(
           ...
           "opciones"=>"<a href='Roles/Editar/$record->id' class='btn btn-azul'>/ Editar</a>&nbsp;<a href='Roles/Permisos/$record->id' class='btn btn-azulclaro'>/ Permisos</a>&nbsp;<button type='button' data-id='$record->id' data-nombre='$record->nombre' class='btn btn-rojo datoselim' data-toggle='modal' data-target='#user-id'>/ Eliminar</button>"
        );
     }
